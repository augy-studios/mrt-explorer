// Service worker for MRT Map Explorer.
//
...
async function fillShell() {
  ...
  if (failed.length) {
    console.warn(
      `[mrtexplorer] ${failed.length} of ${PRECACHE.length} precache entries failed. ` +
...
// by going offline still gets the right font. Other subsets are cached the
// first time a page uses them. Best effort: a failure here costs the font,
// never the install.
...
async function navigation(request, url) {
  ...
  try {
    ...
    // Offline and not a page this worker holds: the map is the only page.
